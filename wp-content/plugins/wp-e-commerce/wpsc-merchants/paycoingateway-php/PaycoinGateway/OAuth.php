...
class PaycoinGateway_OAuth
{
    ...
    {
        $url = "https://www.paycoingateway.com/oauth/authorize?response_type=code" .
    ...
    public function getTokens($code, $grantType='authorization_code')
    {
        ...
        if($response === false) {
            ...
            curl_close($curl);
            throw new PaycoinGateway_ConnectionException("Could not get tokens - network error " . $message . " (" . $error . ")");
        }
        if($statusCode !== 200) {
            throw new PaycoinGateway_ApiException("Could not get tokens - code " . $statusCode, $statusCode, $response);
        }
        curl_close($curl);
        ...
        try {
            ...
            throw new PaycoinGateway_ConnectionException("Could not get tokens - JSON error", $statusCode, $response);
        }
