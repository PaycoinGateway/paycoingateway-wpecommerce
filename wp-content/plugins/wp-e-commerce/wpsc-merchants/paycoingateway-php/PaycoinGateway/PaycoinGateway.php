...
class PaycoinGateway
{
    ...
    {
        return new PaycoinGateway(new PaycoinGateway_ApiKeyAuthentication($key, $secret));
    }

    public static function withSimpleApiKey($key)
    {
        return new PaycoinGateway(new PaycoinGateway_SimpleApiKeyAuthentication($key));
    }

    public static function withOAuth($oauth, $tokens)
    {
        return new PaycoinGateway(new PaycoinGateway_OAuthAuthentication($oauth, $tokens));
    }
    ...
    public function __construct($authentication, $tokens=null, $apiKeySecret=null)
    {
        ...
        if (is_a($authentication, 'PaycoinGateway_Authentication')) {
            ...
            if ($tokens !== null) {
                $this->_authentication = new PaycoinGateway_OAuthAuthentication($authentication, $tokens);
            } else if ($authentication !== null && is_string($authentication)) {
                $apiKey = $authentication;
                if ($apiKeySecret === null) {
                    // Simple API key
                    $this->_authentication = new PaycoinGateway_SimpleApiKeyAuthentication($apiKey);
                } else {
                    $this->_authentication = new PaycoinGateway_ApiKeyAuthentication($apiKey, $apiKeySecret);
                }
            } else {
                throw new PaycoinGateway_ApiException('Could not determine API authentication scheme');
            }
        }

        $this->_rpc = new PaycoinGateway_Rpc(new PaycoinGateway_Requestor(), $this->_authentication);
    }

    // Used for unit testing only
    public function setRequestor($requestor)
    {
        $this->_rpc = new PaycoinGateway_Rpc($requestor, $this->_authentication);
        return $this;
    }
    ...
    public function createButtonWithOptions($options=array())
    {
        ...
        $returnValue->embedHtml = "<div class=\"PaycoinGateway-button\" data-code=\"" . $response->button->code . "\"></div><script src=\"https://PaycoinGateway.com/assets/button.js\" type=\"text/javascript\"></script>";
